...
module.exports = ({ verify, secret, app, sockets }) => {
  app.get("/subscriptions", verify, async (_, res) => {
    ...
    if (list.status > 399) return handleInvalidPermissions(list.status);
		...
		const { stream, disabled = [], available = [] } = list.data.reduce((acc, sub) => {
    ...
    if (disabled.length > 0)
      await Promise.all(
        disabled.map((sub) => delSub({ auth, subId: sub.id })),
      );
    ...
    res.send(
      ...
      if (events.status > 399) return handleInvalidPermissions(streamSub.status);
		}

		const onlineStatus = await checkOnlineStatus({ userId, auth })
		if (onlineStatus.data.length === 1) 
			sockets[userId].isOnline = true
  });
